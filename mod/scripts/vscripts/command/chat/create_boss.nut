global function ChatCommand_CreateBoss_Init
...
void function ChatCommand_CreateBoss_Init() {
    RegisterSignal( "AshEnteredPhaseShift" ) //艾许相位
    RegisterSignal( "DoCore" ) //毒蛇开核心

    AddServerChatCommandCallback("/create_boss",  ChatCommand_CreateBoss)
    AddServerChatCommandCallback("/kill_all_boss",  ChatCommand_KillAllBoss)
}
...
void function ChatCommand_CreateBoss(entity player, array<string> args)
{
    ...
    Leaf_ChatServerPrivateMessage(player, "创建成功！！！")
}
...
void function CreateAsh(vector origin, vector angles) {
    entity ash = CreateNPCTitan("npc_titan_stryder_leadwall", 2, origin, < 0, angles.y, 0 >) //angles < x, y, z >
...
void function CreateViper(vector origin, vector angles) {
    entity viper = CreateNPCTitan("npc_titan_stryder", 2, origin, < 0, angles.y, 0 >) //angles < x, y, z >
...
void function CreateRichter(vector origin, vector angles) {
    entity richter = CreateNPCTitan("npc_titan_atlas_tracker", 2, origin, < 0, angles.y, 0 >) //angles < x, y, z >
...
void function CreateSlone(vector origin, vector angles) {
    entity slone = CreateNPCTitan("npc_titan_atlas_stickybomb", 2, origin, < 0, angles.y, 0 >) //angles < x, y, z >
...
void function CreateKane(vector origin, vector angles) {
    entity kane = CreateNPCTitan("npc_titan_ogre_meteor", 2, origin, < 0, angles.y, 0 >) //angles < x, y, z >
...
void function CreateBlisk(vector origin, vector angles) {
    entity blisk = CreateNPCTitan("npc_titan_ogre_minigun", 2, origin, < 0, angles.y, 0 >) //angles < x, y, z >
